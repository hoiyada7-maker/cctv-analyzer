...
다음 6가지 행동만 찾아줘:
1. 기저귀체크: 간병인이 환자의 기저귀 상태를 확인하는 행동 (이불 들추기, 하체 확인 등)
2. 기저귀교체: 간병인이 환자의 기저귀를 실제로 교체하는 행동 (기저귀 제거, 새 기저귀 착용, 엉덩이 닦기 등)
3. 위생케어: 수건·물티슈 등으로 환자의 얼굴·손·몸을 닦는 행동
4. 환자분노: 환자가 간병인에게 화를 내거나 소리를 지르거나 밀치는 행동
5. 간병인학대: 간병인이 환자를 때리거나 밀거나 고함을 치는 행동
6. 환자발버둥: 환자가 크게 몸을 움직이거나 발버둥치거나 침대에서 벗어나려는 행동
...
응답 형식:
{
  ...
  "events": [
    {"time": "MM:SS", "category": "기저귀체크|기저귀교체|위생케어|환자분노|간병인학대|환자발버둥", "description": "한국어 1문장"}
  ],
  "summary": "전체 영상을 한 문장으로 (한국어)"
}

규칙:
- 위 6가지 외의 행동은 이벤트에 포함하지 않음
